    ...
    public class UserController : AuthenticatedControllerBase
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (registered != null)
            {
                ...
                if (!string.IsNullOrEmpty(registered.PreferredCulture))
                {
                    // Set the standard ASP.NET Core localization cookie so
                    // CookieRequestCultureProvider picks it up on the next request.
                    // Mutating CultureInfo.CurrentCulture directly is unsafe on
                    // a shared thread-pool thread and does not survive redirects.
                    Response.Cookies.Append(
                        Microsoft.AspNetCore.Localization.CookieRequestCultureProvider.DefaultCookieName,
                        Microsoft.AspNetCore.Localization.CookieRequestCultureProvider.MakeCookieValue(
                            new Microsoft.AspNetCore.Localization.RequestCulture(registered.PreferredCulture)),
                ...
                if (!string.IsNullOrEmpty(registered.PreferredCurrency))
                {
                    Response.Cookies.Append("appCurrency", registered.PreferredCurrency, new Microsoft.AspNetCore.Http.CookieOptions
                    {
                        Expires     = System.DateTimeOffset.UtcNow.AddYears(1),
                        IsEssential = true,
                        SameSite    = Microsoft.AspNetCore.Http.SameSiteMode.Lax,
                        Secure      = true  // Only transmit over HTTPS; not HttpOnly so JS can read the UI preference
                    });
                }
            }
